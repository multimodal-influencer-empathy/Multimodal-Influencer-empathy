...
class A6_EF_LSTM_Audio_Image(nn.Module):
    """
    early fusion using lstm
    """

    def __init__(self, args):
        super(A6_EF_LSTM_Audio_Image, self).__init__()
        text_in, audio_in, vision_in = args.feature_dims

        in_size = audio_in + vision_in
        ...
        self.lstm = nn.LSTM(in_size, hidden_size, num_layers=num_layers, dropout=dropout, bidirectional=True,
    ...
    def forward(self, text_x, audio_x, vision_x):
        x = torch.cat([audio_x, vision_x], dim=-1)
        x = self.norm(x)
